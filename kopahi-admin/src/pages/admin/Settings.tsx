...
type TabProps = { settings: Settings; update: UpdateFn };
...
function SecurityTab({ settings, update }: TabProps) {
  return (
    <>
          ...
          <div key={i.name} className="border border-gray-100 rounded-xl p-5 hover:border-green-200 transition-colors">
            ...
            <button
              className={`w-full text-sm font-medium py-2 rounded-lg transition-colors ${
                i.connected ? "bg-gray-100 hover:bg-gray-200 text-gray-700" : "bg-green-700 hover:bg-green-800 text-white"
              }`}
            >
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3">
                    <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-green-50 text-green-700">{inv.status}</span>
                  </td>
...
function AuditTab() {
  ...
  return (
    <Section title="Audit Log" desc="Track every important action taken on the platform.">
  ...
  return (
    <div>
      ...
      {prefix || suffix ? (
        <div className="flex">
          {prefix && (
            <span className="inline-flex items-center px-3 bg-gray-50 border border-r-0 border-gray-200 rounded-l-lg text-sm text-gray-600 font-medium">
              {prefix}
            </span>
          )}
          <div
            className={`flex-1 ${prefix ? "[&>input]:rounded-l-none" : ""} ${suffix ? "[&>input]:rounded-r-none" : ""}`}
          >
            {children}
          </div>
          {suffix && (
            <span className="inline-flex items-center px-3 bg-gray-50 border border-l-0 border-gray-200 rounded-r-lg text-sm text-gray-600 font-medium">
              {suffix}
            </span>
          )}
        </div>
  ...
  return (
    <label className="flex items-start justify-between gap-4 py-3 cursor-pointer hover:bg-gray-50 -mx-3 px-3 rounded-lg transition-colors">
      ...
      <button
        ...
        className={`relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full transition-colors focus:outline-none focus:ring-4 focus:ring-green-100 mt-0.5 ${
